...
// ── OGG Vorbis loader (via stb_vorbis) ─────────────────────────────────────
// Uses Sean Barrett's stb_vorbis for full, correct Vorbis decoding.
// This replaces the previous incomplete custom decoder.

// Suppress warnings from third-party single-header library
...
// stb_vorbis.c is a single-file library — include implementation here
...
bool load_ogg_from_memory(const u8* data, size_t size, AudioBuffer& out_buffer) {
    ...
    // stb_vorbis_decode_memory decodes the entire OGG Vorbis stream to
    // interleaved 16-bit PCM and returns the number of sample frames.
    int frame_count = stb_vorbis_decode_memory(
        data, static_cast<int>(size),
        &channels, &sample_rate, &decoded);

    if (frame_count <= 0 || decoded == nullptr) {
        NX_ERROR("OGG: stb_vorbis failed to decode (returned {})", frame_count);
        if (decoded) free(decoded);
        return false;
    }

    // Convert interleaved S16 to our AudioBuffer format
    out_buffer.format.sample_rate = static_cast<u32>(sample_rate);
    out_buffer.format.channels = static_cast<u32>(channels);
    out_buffer.format.format = SampleFormat::S16;
    out_buffer.frame_count = static_cast<u32>(frame_count);

    size_t total_samples = static_cast<size_t>(frame_count) * static_cast<size_t>(channels);
    out_buffer.data.resize(total_samples * sizeof(short));
    std::memcpy(out_buffer.data.data(), decoded, total_samples * sizeof(short));

    free(decoded);

    NX_INFO("OGG: decoded Vorbis stream ({}ch, {}Hz, {:.2f}s)",
            channels, sample_rate,
            static_cast<float>(frame_count) / static_cast<float>(sample_rate));
